...
def get_dummy_loaders(batch_size=8):
    ...
    unlabeled_ds = TensorDataset(x_unlabeled, torch.zeros_like(y_labeled))
...
def test_train_computes_gradients():
    ...
    assert any(g.abs().sum().item() > 0 for g in grads), "Aucun gradient calculé"
